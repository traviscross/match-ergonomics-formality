...
impl ShowType {
  ...
  fn from_str(conf: Conf, xs: &str) -> Result<Self, Error> {
    Ok(Self { conf, stmt: LetStmt::from_str(xs)? })
  }
  ...
  fn show_for<W: Write>(
    ...
    let mut r = Reduction::from_stmt(conf, self.stmt.clone());
    write!(f, "//~| {}: ", label)?;
    match r.to_type() {
  ...
  fn show_for_current<W: Write>(
    ...
    let mut r = Reduction::from_stmt(self.conf, self.stmt.clone());
    write!(f, "//~| {}: ", label)?;
    match r.to_type() {
      Ok((ident, ty)) => writeln!(f, "{ident}: {ty}")?,
      Err(e) => writeln!(f, "ERROR {}", e)?,
...
impl Display for ShowType {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    self.show_for(f, conf, "* Rust stable == RFC 2005")?;
    writeln!(f, /**/ r"//~| |\");
    conf.rule3 = true;
    ...
    conf.rule5 = true;
    self.show_for(f, conf, "| * Rule 5 == RFC 3627 Rust ..2021")?;
    let mut conf = Conf::default();
    conf.rule1 = true;
    ...
    self.show_for(f, conf, "| * Rule 4 (early) == waffle")?;
    let mut conf = Conf::default();
    conf.rule1 = true;
    conf.rule2 = true;
    ...
    self.show_for(f, conf, "| * Rule 3 (lazy)")?;
    conf.rule4_early = true;
    self.show_for(f, conf, "| * Rule 4 (early)")?;
    conf.spin = true;
    self.show_for(f, conf, "| * Spin rule == rpjohnst")?;
    let mut conf = Conf::default();
    conf.rule1 = true;
    conf.rule2 = true;
    conf.rule3 = true;
    ...
    self.show_for(f, conf, "* Rule 4")?;
    conf.rule4_ext = true;
    self.show_for(f, conf, "* Rule 4 (ext)")?;
    writeln!(f, /**/ r"//~| |\");
    conf.rule4_ext2 = true;
    self.show_for(f, conf, "| * Rule 4 (ext2)")?;
    conf.rule5 = true;
    self.show_for(f, conf, "| * Rule 5")?;
    conf.rule4_ext2 = false;
    self.show_for(f, conf, "* Rule 5 == RFC 3627 Rust 2024")?;
    Ok(())
  }
}
  ...
  loop {
    match rl.readline(">> ") {
      ...
      Ok(line) if line.trim_start().starts_with("let ") => {
        ...
        print!("{:#}", ShowType::from_stmt(conf, stmt));
      }
      ...
      Ok(line) if line.trim() == "show" => {
        ...
        println!("| rule4_ext: {}", conf.rule4_ext);
        println!("| rule4_ext2: {}", conf.rule4_ext2);
      ...
      Ok(line) if line.trim() == "set waffle" => {
        rl.add_history_entry(line.as_str());
        conf = Conf::rust_2024_waffle();
      }
      ...
      Ok(line) if line.trim() == "set rule4_ext" => {
        rl.add_history_entry(line.as_str());
        conf.rule4_ext = true;
      }
      Ok(line) if line.trim() == "unset rule4_ext" => {
        rl.add_history_entry(line.as_str());
        conf.rule4_ext = false;
      }
      Ok(line) if line.trim() == "set rule4_ext2" => {
        rl.add_history_entry(line.as_str());
        conf.rule4_ext2 = true;
      }
      Ok(line) if line.trim() == "unset rule4_ext2" => {
        rl.add_history_entry(line.as_str());
        conf.rule4_ext2 = false;
      }
